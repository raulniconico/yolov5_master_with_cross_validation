...
class DataLoader:
    ...
    def distribute_k_folder(self):
        ...
        for image in images[0: self.train_number]:
            ...
            copyfile(self.path + "/" + self.train + "/images/" + image, "data/my_data/train/images/" + image)
            copyfile(self.path + "/" + self.train + "/labels/" + os.path.splitext(image)[0] + ".txt",
        ...
        for image in images[self.train_number + 1: self.train_number + self.valid_number]:
            copyfile(self.path + "/" + self.train + "/images/" + image, "data/my_data/valid/images/" + image)
            copyfile(self.path + "/" + self.train + "/labels/" + os.path.splitext(image)[0] + ".txt",
                     "data/my_data/valid/labels/" + os.path.splitext(image)[0] + ".txt")

        for image in images[self.train_number + self.valid_number + 1: -1]:
            copyfile(self.path + "/" + self.train +"/images/" + image, "data/my_data/test/images/" + image)
            copyfile(self.path + "/" + self.train +"/labels/" + os.path.splitext(image)[0] + ".txt",
